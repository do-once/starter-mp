...
const isStartsWithHttp = (url: string) =>/** http/https */ url.indexOf('http') === 0 || /** 相对协议 */url.indexOf('//') === 0
...
export class Request {
    ...
    private _fetch<D>(requestOption: Omit<WechatMiniprogram.RequestOption, 'success' | 'fail'>) {

        return new Promise<D | undefined>((resolve) => {
            wx.request<MyApp.IRes<D>>({
                ...requestOption,
